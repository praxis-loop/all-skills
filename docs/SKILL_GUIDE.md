...
```text
skills/<function>/<domain>/<skill-name>/
├── SKILL.md
├── references/
├── scripts/
└── assets/
...
- `function` 是顶层职能，回答“这件事属于哪个部门？”例如 `engineering`、`marketing`、`legal`。
- `domain` 是职能内主题，回答“这个部门里的哪个领域？”例如 `engineering/security`、`marketing/seo`、`legal/contracts`。
- `skill-name` 是具体能力，回答“这个 skill 具体做什么？”例如 `cloudflare-tunnel-debug`。
- 顶层职能优先使用 `docs/CATEGORIES.md` 中已有分类。
- 新增顶层职能前，应确认该方向未来会沉淀多个 domain 和多个 skill。
- 新增 domain 前，应确认该主题有稳定工作流、模板、脚本、检查项或安全边界。

## 命名规则

- `function`、`domain`、`skill-name` 都必须使用小写字母、数字和短横线。
- 避免使用 `_`、空格、大写字母和中文目录名。
- 避免让 `skill-name` 以 `-skill` 结尾，目录本身已经表达这是 skill。
...
3. 选择 function 和 domain：优先使用 `docs/CATEGORIES.md` 中已有分类。
4. 创建目录：`skills/<function>/<domain>/<skill-name>/SKILL.md`。
...
```bash
cp -R templates/new-skill skills/<function>/<domain>/<skill-name>
```

然后修改 `skills/<function>/<domain>/<skill-name>/SKILL.md`。
...
- 分类准确：function、domain 和 skill-name 能让未来的自己快速找到。
