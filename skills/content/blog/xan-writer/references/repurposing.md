...
- `xan-writer` 负责选题、口播逐字稿和拍摄提示；`xan-broll` 只接收已经核验的口播与画面命题，不反向修改事实和口播判断。
...
衍生稿与母稿一起放在 `<vault>/10-创作/20-草稿/`，文件名增加 `-口播稿`。它不是第二份母稿，frontmatter 至少包含：
...
需要做画面时，把口播稿路径、中心判断、拍摄节奏和素材清单交给 `xan-broll`。真人出镜与录屏由拍摄计划承担；`xan-broll` 只补适合的视觉隐喻或讲解片。
