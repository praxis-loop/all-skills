---
name: xan-obsidian
description: 在用户本地新建、检查或渐进适配一个面向长期知识与内容生产的 Obsidian 知识库，复用 Xan 的资料、知识、灵感、成稿包、调研、草稿、发布与回流分层。用于“帮我搭一个 Obsidian 知识库”“给 xan-writer 准备本地知识库”“把现有 vault 接入内容生产管线”“检查知识库结构是否完整”时；也在 xan-writer 找不到兼容知识库时使用。只补缺失结构，不覆盖、移动或删除用户已有笔记，不强制安装插件、同步服务或 Git。
---
...
1. 不默认使用 `xans-brain`，也不假设知识库位于 iCloud、用户主目录或当前项目。
...
1. 用户明确给出的目录。
2. 当前目录或父目录中的 `.xan-obsidian.json`。
3. 当前目录或父目录中同时存在 `10-创作/`、`20-知识/`、`30-资料/`、`50-系统/` 的兼容知识库。
...
- 新建 `.xan-obsidian.json` 供其他 Xan Skill 发现结构版本。
...
- `00-入口/10-工作台/开始这里.md`
- `50-系统/30-模板/内容成稿包.md`
- `50-系统/20-流程/个人知识库工作流.md`
...
- 新资料放入 `30-资料/`。
- 想推进的主题在 `10-创作/10-灵感/20-成稿包/` 建立成稿包。
- 完整草稿进入 `10-创作/20-草稿/`，确认发布后才进入 `40-发布/`。

用户接着要求写文章时，把知识库根目录、材料路径和目标平台交给 `xan-writer`。不要复制整套知识库到 Skill 目录，也不要把用户笔记吸收成 Skill 样本。
