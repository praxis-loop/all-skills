...
def load_manifest(root):
    manifest_path = root / ".xan-obsidian.json"
...
def write_text_safely(destination, text):
    temporary = destination.with_name(destination.name + ".xan-obsidian-tmp")
...
def initialize(root, vault_name, dry_run):
    ...
    if not existing_manifest:
        manifest = {
            ...
            "name": vault_name,
            "created_at": utc_now(),
            "managed_by": "xan-obsidian",
        }
        write_text_safely(
            root / ".xan-obsidian.json",
...
def main():
    parser = argparse.ArgumentParser(description="Safely initialize or check a Xan-style Obsidian vault")
